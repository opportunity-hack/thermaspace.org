...
class FlirOne(
    ...
    private fun readLoop() {
        ...
        while (running) {
            ...
            val data = chunk.copyOf(n)
            buf = if (startsWithMagic(data) || buf.size > (4 shl 20)) data else buf + data
            if (!startsWithMagic(buf) || buf.size < 28) continue

            val frameSize = le32(buf, 8)
            if (buf.size < 28 + frameSize) continue
            if (frames < 3) {
                frames++
                log("frame #$frames: frameSize=$frameSize thermal=${le32(buf, 12)} jpg=${le32(buf, 16)} status=${le32(buf, 20)}")
            }
            parse(buf, frameSize)?.let(onFrame)
            buf = ByteArray(0)
        }
        log("read loop exited")
    }

    private fun startsWithMagic(b: ByteArray) =
        b.size >= 4 && b[0] == MAGIC[0] && b[1] == MAGIC[1] && b[2] == MAGIC[2] && b[3] == MAGIC[3]

    private fun le32(b: ByteArray, o: Int): Int =
        (b[o].toInt() and 0xFF) or ((b[o + 1].toInt() and 0xFF) shl 8) or
        ((b[o + 2].toInt() and 0xFF) shl 16) or ((b[o + 3].toInt() and 0xFF) shl 24)

    private fun parse(buf: ByteArray, frameSize: Int): FlirFrame? {
        val thermalSize = le32(buf, 12)
        val jpgSize = le32(buf, 16)
        val statusSize = le32(buf, 20)
        if (28 + thermalSize + jpgSize + statusSize > buf.size) return null
